...
#
# Models adjusted for:
# Age, sex, BMI, and estimated immune cell proportions.
#
...
### ------------------------------------------------------------------
### Load normalized methylation data (AADM)
### ------------------------------------------------------------------

# Normalized beta and M-values
beta <- read.table(
  "/project/Karlijn/EWAS_Adiponectin/Normalisation/20241025_Beta_Funnurm_NoSex_SNPs_NoCrosshybrAADM_Adiponectin.txt",
  sep = "\t", header = TRUE
)

mval2 <- read.table(
  "/project/Karlijn/EWAS_Adiponectin/Normalisation/20241025_Mval_Funnurm_NoSex_SNPs_NoCrosshybrAADM_Adiponectin.txt",
  sep = "\t", header = TRUE
)

# Probe annotation
probe.features <- read.table(
  "/project/Karlijn/EWAS_Adiponectin/Normalisation/20241025_Annot_NoXY_NoSNP_NoCrosshyb_Adiponectin_AADM.txt",
  sep = "\t", header = TRUE
)
...
phenofile <- "/project/Karlijn/EWAS_Adiponectin/Pheno/2024.10.22_adiponectin_model_n593_wCells.csv"
...
write.table(
  bumps.annotated,
  "2025.03.25_DMRs_Bumphunter_AADM_Adiponectin_AgeSexCellBMI_n593.txt",
  sep = "\t",
  quote = FALSE,
  row.names = FALSE
)
